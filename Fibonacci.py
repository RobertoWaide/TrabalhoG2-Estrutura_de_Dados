...
class FibonacciHeap():
    ...
    def consolidate(self): #Organization system that fixes 
    ...
    def decrease_key(self, x, k): #selects the node that you want to decrease and to which number will it be decreased
        if k > x.key:
            return None
        x.key = k
        y = x.parent
        if y is not None and x.key < y.key:
            self.cut(x, y) #cut it from its father and add to the root list
            self.cascading_cut(y) #activates the marked variable, if the marked variable is already active it goes to the root list
        if x.key < self.min.key: #compares if the the changed node is now the lowest
            self.min = x
    ...
    def delete(self, x):
        self.decrease_key(x, float('-inf')) #define node as the lowest and send it to root list
        return self.extract_min() #takes it off the root list
